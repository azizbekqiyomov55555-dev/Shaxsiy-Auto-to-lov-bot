...
async def jsonbin_load() -> dict:
    """JSONBin dan barcha ma'lumotlarni yuklash"""
    headers = {
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                ...
                if resp.status == 200:
                    ...
                    logging.info("JSONBin: Ma'lumotlar yuklandi")
                    return data
                else:
                    logging.warning(f"JSONBin yuklash xato: {resp.status}")
                    return {}
    except Exception as e:
        logging.error(f"JSONBin yuklash xato: {e}")
        return {}


async def jsonbin_save(data: dict) -> bool:
    """JSONBin ga barcha ma'lumotlarni saqlash"""
    headers = {
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.put(
                f"{JSONBIN_BASE_URL}/b/{JSONBIN_BIN_ID}",
                json=data,
                headers=headers,
                timeout=aiohttp.ClientTimeout(total=15)
            ) as resp:
                if resp.status == 200:
                    logging.info("JSONBin: Ma'lumotlar saqlandi")
                    return True
                else:
                    logging.warning(f"JSONBin saqlash xato: {resp.status}")
                    return False
    except Exception as e:
        logging.error(f"JSONBin saqlash xato: {e}")
        return False


async def collect_all_data() -> dict:
    """SQLite dan barcha muhim ma'lumotlarni yig'ish"""
    users = db_execute("SELECT * FROM users", fetch=True)
    channels = db_execute("SELECT * FROM channels", fetch=True)
    settings = db_execute("SELECT * FROM settings", fetch=True)
    uc_prices = db_execute("SELECT * FROM uc_prices ORDER BY uc_amount ASC", fetch=True)
    uc_orders = db_execute("SELECT * FROM uc_orders ORDER BY id DESC LIMIT 500", fetch=True)
    stars_prices = db_execute("SELECT * FROM stars_prices ORDER BY stars_amount ASC", fetch=True)
    stars_orders = db_execute("SELECT * FROM stars_orders ORDER BY id DESC LIMIT 500", fetch=True)
    premium_prices = db_execute("SELECT * FROM premium_prices ORDER BY price ASC", fetch=True)
    premium_orders = db_execute("SELECT * FROM premium_orders ORDER BY id DESC LIMIT 500", fetch=True)
    pending_payments = db_execute("SELECT * FROM pending_payments WHERE status='pending'", fetch=True)
    ads = db_execute("SELECT * FROM ads ORDER BY id DESC LIMIT 200", fetch=True)
    ...
    return {
        ...
        "channels": rows_to_list(channels),
        "settings": rows_to_list(settings),
        "uc_prices": rows_to_list(uc_prices),
        "uc_orders": rows_to_list(uc_orders),
        "stars_prices": rows_to_list(stars_prices),
        "stars_orders": rows_to_list(stars_orders),
        "premium_prices": rows_to_list(premium_prices),
        "premium_orders": rows_to_list(premium_orders),
        "pending_payments": rows_to_list(pending_payments),
        "ads": rows_to_list(ads),
        "last_updated": get_time_tashkent()
    }


async def restore_from_jsonbin():
    """JSONBin dan SQLite ga ma'lumotlarni tiklash"""
    logging.info("JSONBin dan ma'lumotlar tiklanmoqda...")
    data = await jsonbin_load()
    if not data:
        logging.info("JSONBin bo'sh, tiklash o'tkazib yuborildi.")
        return

    def safe_list(val):
        """Har qanday formatdagi qiymatni xavfsiz list ga aylantirish"""
        if isinstance(val, list):
            return val
        if isinstance(val, dict):
            # {"key": "value", ...} formatidagi settings uchun
            return [{"key": k, "value": v} for k, v in val.items()]
        return []

    def safe_get(obj, key, default=None):
        """dict yoki boshqa objectdan xavfsiz qiymat olish"""
    ...
    try:
        # Users
        for u in safe_list(data.get("users", [])):
            try:
                c.execute("""INSERT OR IGNORE INTO users
                    (user_id, full_name, username, join_date, posted_ads, paid_slots, pending_approval, free_ad_used)
                    VALUES (?,?,?,?,?,?,?,?)""",
                    (safe_get(u, "user_id"), safe_get(u, "full_name", ""),
                     safe_get(u, "username", ""), safe_get(u, "join_date", ""),
                     safe_get(u, "posted_ads", 0), safe_get(u, "paid_slots", 0),
                     safe_get(u, "pending_approval", 0), safe_get(u, "free_ad_used", 0)))
            except Exception as e:
                logging.warning(f"User tiklash xato: {e}")

        # Channels
        c.execute("DELETE FROM channels")
        for ch in safe_list(data.get("channels", [])):
            try:
                c.execute("INSERT OR IGNORE INTO channels (channel_id, url) VALUES (?,?)",
                          (safe_get(ch, "channel_id"), safe_get(ch, "url")))
            except Exception as e:
                logging.warning(f"Channel tiklash xato: {e}")

        # Settings — list yoki dict formatda kelishi mumkin
        raw_settings = data.get("settings", [])
        if isinstance(raw_settings, dict):
            # {"price": "50000", "card": "...", ...} formatida kelgan
            settings_list = [{"key": k, "value": v} for k, v in raw_settings.items()]
        else:
            settings_list = raw_settings if isinstance(raw_settings, list) else []
        ...
        for s in settings_list:
            try:
                ...
                logging.warning(f"Setting tiklash xato: {e}")

        # UC prices
        c.execute("DELETE FROM uc_prices")
        for p in safe_list(data.get("uc_prices", [])):
            try:
                c.execute("INSERT OR IGNORE INTO uc_prices (uc_amount, price, position) VALUES (?,?,?)",
                          (safe_get(p, "uc_amount"), safe_get(p, "price"), safe_get(p, "position", 0)))
            except Exception as e:
                logging.warning(f"UC price tiklash xato: {e}")

        # Stars prices
        c.execute("DELETE FROM stars_prices")
        for p in safe_list(data.get("stars_prices", [])):
            try:
                c.execute("INSERT OR IGNORE INTO stars_prices (stars_amount, price, position) VALUES (?,?,?)",
                          (safe_get(p, "stars_amount"), safe_get(p, "price"), safe_get(p, "position", 0)))
            except Exception as e:
                logging.warning(f"Stars price tiklash xato: {e}")

        # Premium prices
        c.execute("DELETE FROM premium_prices")
        for p in safe_list(data.get("premium_prices", [])):
            try:
                c.execute("INSERT OR IGNORE INTO premium_prices (duration, price, position) VALUES (?,?,?)",
                          (safe_get(p, "duration"), safe_get(p, "price"), safe_get(p, "position", 0)))
            except Exception as e:
                logging.warning(f"Premium price tiklash xato: {e}")

        # UC orders
        for o in safe_list(data.get("uc_orders", [])):
            try:
                c.execute("""INSERT OR IGNORE INTO uc_orders
                    (id, user_id, full_name, username, uc_amount, price, pubg_id,
                     screenshot_id, status, payment_method, payment_id, order_date)
                    VALUES (?,?,?,?,?,?,?,?,?,?,?,?)""",
                    (safe_get(o, "id"), safe_get(o, "user_id"),
                     safe_get(o, "full_name", ""), safe_get(o, "username", ""),
                     safe_get(o, "uc_amount"), safe_get(o, "price"),
                     safe_get(o, "pubg_id", ""), safe_get(o, "screenshot_id"),
                     safe_get(o, "status", "pending"), safe_get(o, "payment_method", "manual"),
                     safe_get(o, "payment_id", 0), safe_get(o, "order_date", "")))
            except Exception as e:
                logging.warning(f"UC order tiklash xato: {e}")

        # Stars orders
        for o in safe_list(data.get("stars_orders", [])):
            try:
                c.execute("""INSERT OR IGNORE INTO stars_orders
                    (id, user_id, full_name, username, stars_amount, price, target_type,
                     target_username, receipt_id, status, payment_method, payment_id, order_date)
                    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                    (safe_get(o, "id"), safe_get(o, "user_id"),
                     safe_get(o, "full_name", ""), safe_get(o, "username", ""),
                     safe_get(o, "stars_amount"), safe_get(o, "price"),
                     safe_get(o, "target_type", "me"), safe_get(o, "target_username", ""),
                     safe_get(o, "receipt_id"), safe_get(o, "status", "pending"),
                     safe_get(o, "payment_method", "manual"), safe_get(o, "payment_id", 0),
                     safe_get(o, "order_date", "")))
            except Exception as e:
                logging.warning(f"Stars order tiklash xato: {e}")

        # Premium orders
        for o in safe_list(data.get("premium_orders", [])):
            try:
                c.execute("""INSERT OR IGNORE INTO premium_orders
                    (id, user_id, full_name, username, duration, price, target_username,
                     receipt_id, status, payment_method, payment_id, order_date)
                    VALUES (?,?,?,?,?,?,?,?,?,?,?,?)""",
                    (safe_get(o, "id"), safe_get(o, "user_id"),
                     safe_get(o, "full_name", ""), safe_get(o, "username", ""),
                     safe_get(o, "duration", ""), safe_get(o, "price"),
                     safe_get(o, "target_username", ""), safe_get(o, "receipt_id"),
                     safe_get(o, "status", "pending"), safe_get(o, "payment_method", "manual"),
                     safe_get(o, "payment_id", 0), safe_get(o, "order_date", "")))
            except Exception as e:
                logging.warning(f"Premium order tiklash xato: {e}")

        # Pending payments
        for p in safe_list(data.get("pending_payments", [])):
            try:
                c.execute("""INSERT OR IGNORE INTO pending_payments
                    (id, payment_id, user_id, full_name, username, amount, type, status, created_at, order_data)
                    VALUES (?,?,?,?,?,?,?,?,?,?)""",
                    (safe_get(p, "id"), safe_get(p, "payment_id", 0),
                     safe_get(p, "user_id"), safe_get(p, "full_name", ""),
                     safe_get(p, "username", ""), safe_get(p, "amount"),
                     safe_get(p, "type", "ad"), safe_get(p, "status", "pending"),
                     safe_get(p, "created_at", ""), safe_get(p, "order_data", "{}")))
            except Exception as e:
                logging.warning(f"Pending payment tiklash xato: {e}")

        conn.commit()
        logging.info("JSONBin dan tiklash muvaffaqiyatli!")
    except Exception as e:
        logging.error(f"restore_from_jsonbin umumiy xato: {e}")
    finally:
        conn.close()


async def auto_backup():
    """Har 5 daqiqada JSONBin ga saqlash"""
    logging.info("Auto-backup ishga tushdi (har 5 daqiqa)")
    await asyncio.sleep(30)  # Botni ishga tushirishdan 30 soniya keyin
    while True:
        try:
            data = await collect_all_data()
            await jsonbin_save(data)
        except Exception as e:
            logging.error(f"Auto-backup xato: {e}")
...
def get_admin_menu():
    return ReplyKeyboardMarkup(
        keyboard=[
            [KeyboardButton(text="Statistika", style="primary"), KeyboardButton(text="Start xabar", style="primary")],
            [KeyboardButton(text="E'lon narxi", style="success"), KeyboardButton(text="Karta", style="success")],
            [KeyboardButton(text="Kanal qo'shish", style="success"), KeyboardButton(text="Kanal o'chirish", style="danger")],
            [KeyboardButton(text="UC sozlamalari", style="primary"), KeyboardButton(text="Stars sozlamalari", style="primary")],
            [KeyboardButton(text="Premium sozlamalari", style="primary"), KeyboardButton(text="Buyurtmalar", style="success")],
            [KeyboardButton(text="Hammaga xabar yuborish", style="danger")],
...
def get_uc_admin_menu():
    return ReplyKeyboardMarkup(
        keyboard=[
            [KeyboardButton(text="UC narxi qo'shish", style="success"), KeyboardButton(text="UC narxlari", style="primary")],
            [KeyboardButton(text="UC buyurtmalar", style="primary"), KeyboardButton(text="UC narxlarini tozalash", style="danger")],
            [KeyboardButton(text="Admin menyu", style="primary")],
        ],
        resize_keyboard=True,
    )
...
def get_stars_admin_menu():
    return ReplyKeyboardMarkup(
        keyboard=[
            [KeyboardButton(text="Stars narxi qo'shish", style="success"), KeyboardButton(text="Stars narxlari", style="primary")],
            [KeyboardButton(text="Stars buyurtmalar", style="primary"), KeyboardButton(text="Stars narxlarini tozalash", style="danger")],
            [KeyboardButton(text="Admin menyu", style="primary")],
        ],
        resize_keyboard=True,
    )
...
def get_premium_admin_menu():
    return ReplyKeyboardMarkup(
        keyboard=[
            [KeyboardButton(text="Premium narxi qo'shish", style="success"), KeyboardButton(text="Premium narxlari", style="primary")],
            [KeyboardButton(text="Premium buyurtmalar", style="primary"), KeyboardButton(text="Premium narxlarini tozalash", style="danger")],
            [KeyboardButton(text="Admin menyu", style="primary")],
        ],
        resize_keyboard=True,
    )
...
def get_orders_admin_menu():
    return ReplyKeyboardMarkup(
        keyboard=[
            [KeyboardButton(text="UC buyurtmalar", style="primary"), KeyboardButton(text="Stars buyurtmalar", style="primary")],
            [KeyboardButton(text="Premium buyurtmalar", style="primary")],
            [KeyboardButton(text="Admin menyu", style="success")],
        ],
        resize_keyboard=True,
    )
...
def get_uc_prices_keyboard(page=0):
    ...
    nav_row = []
    if page > 0:
        nav_row.append(InlineKeyboardButton(text="Oldingi", callback_data=f"uc_page_{page - 1}", style="primary"))
    if page < total_pages - 1:
        nav_row.append(InlineKeyboardButton(text="Keyingi", callback_data=f"uc_page_{page + 1}", style="primary"))
    if nav_row:
        rows.append(nav_row)
...
def get_stars_prices_keyboard(page=0):
    ...
    nav_row = []
    if page > 0:
        nav_row.append(InlineKeyboardButton(text="Oldingi", callback_data=f"stars_page_{page - 1}", style="primary"))
    if page < total_pages - 1:
        nav_row.append(InlineKeyboardButton(text="Keyingi", callback_data=f"stars_page_{page + 1}", style="primary"))
    if nav_row:
        rows.append(nav_row)
...
def get_premium_prices_keyboard(page=0):
    ...
    nav_row = []
    if page > 0:
        nav_row.append(InlineKeyboardButton(text="Oldingi", callback_data=f"premium_page_{page - 1}", style="primary"))
    if page < total_pages - 1:
        nav_row.append(InlineKeyboardButton(text="Keyingi", callback_data=f"premium_page_{page + 1}", style="primary"))
    if nav_row:
        rows.append(nav_row)
...
async def admin_uc_list_btn(message: Message):
    ...
    for item in prices:
        ...
        rows.append([
            InlineKeyboardButton(text=f"{item['uc_amount']} UC", style="primary", callback_data="uc_info"),
            InlineKeyboardButton(text="O'chirish", callback_data=f"del_uc_price_{item['id']}", style="danger"),
        ])
...
async def admin_stars_list_btn(message: Message):
    ...
    for item in prices:
        ...
        rows.append([
            InlineKeyboardButton(text=f"{item['stars_amount']} Stars", callback_data="stars_info", style="primary"),
            InlineKeyboardButton(text="O'chirish", callback_data=f"del_stars_price_{item['id']}", style="danger"),
        ])
    rows.append([InlineKeyboardButton(text="Yopish", callback_data="close_list", style="danger")])
    await message.answer(text, parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=rows))
...
async def admin_premium_list_btn(message: Message):
    ...
    for item in prices:
        ...
        rows.append([
            InlineKeyboardButton(text=f"{item['duration']}", callback_data="premium_info", style="primary"),
            InlineKeyboardButton(text="O'chirish", callback_data=f"del_premium_price_{item['id']}", style="danger"),
        ])
    rows.append([InlineKeyboardButton(text="Yopish", callback_data="close_list", style="danger")])
    await message.answer(text, parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=rows))
    ...
    if item:
        ...
        for p in prices:
            ...
            rows.append([
                ...
                InlineKeyboardButton(text="O'chirish", callback_data=f"del_uc_price_{p['id']}", style="danger"),
            ])
        rows.append([InlineKeyboardButton(text="Yopish", callback_data="close_list", style="danger")])
        try:
    ...
    if item:
        ...
        for p in prices:
            ...
            rows.append([
                ...
                InlineKeyboardButton(text="O'chirish", callback_data=f"del_stars_price_{p['id']}", style="danger"),
            ])
        rows.append([InlineKeyboardButton(text="Yopish", callback_data="close_list", style="danger")])
        try:
            await call.message.edit_text(text, parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=rows))
    ...
    if item:
        ...
        for p in prices:
            ...
            rows.append([
                ...
                InlineKeyboardButton(text="O'chirish", callback_data=f"del_premium_price_{p['id']}", style="danger"),
            ])
        rows.append([InlineKeyboardButton(text="Yopish", callback_data="close_list", style="danger")])
        try:
            await call.message.edit_text(text, parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=rows))
...
async def main():
    ...
    print("⏳ JSONBin dan ma'lumotlar tiklanmoqda...")
    await restore_from_jsonbin()
    print("✅ JSONBin dan tiklash tugadi!")
    ...
    # JSONBin auto-backup — har 5 daqiqa
    asyncio.create_task(auto_backup())
    print("✅ JSONBin auto-backup ishga tushdi (har 5 daqiqa)!")
