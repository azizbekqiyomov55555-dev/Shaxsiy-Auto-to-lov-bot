...
def init_db():
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS pending_payments (
        ...
        status TEXT DEFAULT 'pending',
        created_at TEXT,        order_data TEXT DEFAULT ''
    )""")
...
async def check_subscription(user_id):
    ...
    for ch in channels:
        try:
            member = await bot.get_chat_member(ch["channel_id"], user_id)            if member.status in ['left', 'kicked']:
...
async def payment_monitor():
    ...
    while True:
        try:
            ...
            for p in pending:
                ...
                if result.get("status") == "success" and result.get("data", {}).get("status") == "paid":
                    ...
                    if pay_type == "ad":
                        ...
                        pubg_id = order_data.get("pubg_id", "—")                        price = order_data.get("price", amount)
                        ...
                        admin_text = (
                            f"UC BUYURTMA — TO'LOV TASDIQLANDI!\n\n"
                            f"{full_name} | @{username}\nID: {user_id}\n\n"
                            f"UC: {uc_amount} UC\nTo'lov: {price:,} so'm (AUTO)\n\n"
                            f"PUBG ID: {pubg_id}\n{now}"
                        )
                        ...
                        try:
                            await bot.send_message(ADMIN_ID, admin_text, reply_markup=btn)
                        except:
                            pass
                        try:
                            await bot.send_message(
                                user_id,
                                f"To'lovingiz avtomatik tasdiqlandi!\n\n{uc_amount} UC tez orada profilingizga yuboriladi.\nPUBG ID: {pubg_id}\n\nAdmin UC ni yuborishini kuting.",
                                reply_markup=get_main_menu()
                            )
                        except Exception as e:
                        ...
                        admin_text = (
                            f"STARS BUYURTMA — TO'LOV TASDIQLANDI!\n\n"
                            f"{full_name} | @{username}\nID: {user_id}\n\n"
                            f"Stars: {stars_amount} Stars\nTo'lov: {stars_price:,} so'm (AUTO)\n\n"                            f"Kimga: {target_text}\n{now}"
                        )
                        btn = InlineKeyboardMarkup(inline_keyboard=[[
                        ...
                        try:
                            await bot.send_message(
                                user_id,
                                f"To'lovingiz avtomatik tasdiqlandi!\n\n{stars_amount} Stars tez orada yuboriladi.\nKimga: {target_text}\n\nAdmin Stars ni yuborishini kuting.",
                                reply_markup=get_main_menu()
                            )
                        except Exception as e:
                        ...
                        admin_text = (
                            f"PREMIUM BUYURTMA — TO'LOV TASDIQLANDI!\n\n"
                            f"{full_name} | @{username}\nID: {user_id}\n\n"
                            f"Muddat: {duration}\nTo'lov: {prem_price:,} so'm (AUTO)\n\n"
                        ...
                        try:
                            await bot.send_message(
                                user_id,
                                f"To'lovingiz avtomatik tasdiqlandi!\n\n{duration} Telegram Premium tez orada ulanadi.\nProfil: @{target_username}\n\nAdmin Premium ni ulashini kuting.",
                                reply_markup=get_main_menu()                            )
                        except Exception as e:
            ...
            logging.error(f"Monitor xatosi: {e}")
...
async def send_auto_payment_link(
    ...
    if result.get("status") == "success":
        ...
        if p_id and p_url:
            ...
            text = (
                ...
                f"To'lov tasdiqlangach bot avtomatik xabar yuboradi.\n\n"
                f"To'lovdan keyin 10-20 soniya kuting."            )
            if is_callback:
                try:
                    await call_or_msg.message.edit_text(text, reply_markup=btn)
                except:
                    await call_or_msg.message.answer(text, reply_markup=btn)
            else:
                await call_or_msg.answer(text, reply_markup=btn)
            return True
    ...
    text = (
        f"Auto to'lov hozir ishlamayapti.\n\n"
        f"Karta raqamiga to'lang:\n{card}\n"
        f"Summa: {amount:,} so'm\n\n"
        f"To'lov qilgach qizil tugmani bosib chek rasmini yuboring."
    )
    if is_callback:
        try:
            await call_or_msg.message.edit_text(text, reply_markup=btn)
...
def get_admin_menu():
    return ReplyKeyboardMarkup(
        keyboard=[
            ...
            [KeyboardButton(text="E'lon narxi", style="primary"), KeyboardButton(text="Karta", style="primary")],
            ...
            [KeyboardButton(text="Premium sozlamalari", style="primary"), KeyboardButton(text="Buyurtmalar", style="primary")],
...
# ================== NARXLAR KLAVIATURASI (INLINE RANGLI) ==================
...
def get_premium_prices_keyboard(page=0):
    ...
    nav_row = []
    if page > 0:
        nav_row.append(InlineKeyboardButton(text="Oldingi", callback_data=f"premium_page_{page - 1}", style="primary"))    if page < total_pages - 1:
...
async def menu_ad_cb(message: Message, state: FSMContext):
    ...
    if not free_ad_used:
        ...
        await message.answer("Birinchi e'lon BEPUL!\n\nIltimos, akkaunt obzori videosini yuboring:")
        await state.set_state(AdForm.video)
        return
    ...
    await message.answer(
        ...
        f"Admin tasdiqlagan — karta orqali, chek yuborish",
        reply_markup=btn
    )
...
async def pay_ad_auto_cb(call: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        price_int = 50000    await send_auto_payment_link(
        call_or_msg=call,
        amount=price_int,
...
async def get_phone(message: Message, state: FSMContext):
    await state.update_data(phone=message.text)    st = await state.get_data()
    text = (
...
async def uc_menu(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "To'lov tasdiqlangandan so'ng UC tez yuboriladi.\n\nUC miqdorini tanlang:",
        reply_markup=get_uc_prices_keyboard(0)
    )
...
async def uc_page_cb(call: CallbackQuery):    page = int(call.data.split("_")[2])
    try:
...
async def uc_pay_auto_cb(call: CallbackQuery, state: FSMContext):    st = await state.get_data()
    uc_amount = st['uc_amount']
    price = st['uc_price']
    pubg_id = st['pubg_id']
...
async def get_uc_receipt(message: Message, state: FSMContext):
    ...
    await bot.send_photo(ADMIN_ID, photo=receipt_id, caption=admin_text, reply_markup=btn)
    await message.answer(
...
async def uc_approve_cb(call: CallbackQuery):
    ...
    if order:
        ...
        await bot.send_message(
            user_id,
            f"Tabriklaymiz! UC profilingizga tushdi!\n\n"
            f"{order['uc_amount']} UC yuborildi!\nXarid uchun rahmat!",
            reply_markup=get_main_menu()
        )
        ...
        try:
            if call.message.photo:
                ...
                await call.message.edit_text(caption + "\n\nTASDIQLANDI — UC YUBORILDI", reply_markup=None)        except:
            pass
...
async def stars_menu(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "To'lov tasdiqlangandan so'ng Stars tez yuboriladi.\n\nStars miqdorini tanlang:",
        reply_markup=get_stars_prices_keyboard(0)
    )
...
async def get_stars_receipt(message: Message, state: FSMContext):
    ...
    await message.answer(
        f"Buyurtmangiz qabul qilindi!\n\n{stars_amount} Stars\n"
        f"Admin chekni ko'rib Stars ni yuboradi.",
        reply_markup=get_main_menu()
    )
    await state.clear()
...
async def stars_approve_cb(call: CallbackQuery):
    if call.from_user.id != ADMIN_ID:        await call.answer("Ruxsat yo'q!", show_alert=True)
        return
    parts = call.data.split("_")
    user_id = int(parts[2])
    order_id = int(parts[3])
    ...
    if order:
        ...
        await bot.send_message(
            user_id,
            f"Tabriklaymiz! Stars yuborildi!\n\n{order['stars_amount']} Stars\nXarid uchun rahmat!",
            reply_markup=get_main_menu()
        )
...
async def premium_menu(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        reply_markup=get_premium_prices_keyboard(0)
    )
...
# PREMIUM AUTO TO'LOV@router.callback_query(F.data == "premium_pay_auto", PremiumOrderForm.payment_choice)
...
async def get_premium_receipt(message: Message, state: FSMContext):
    ...
    await bot.send_photo(ADMIN_ID, photo=receipt_id, caption=admin_text, reply_markup=btn)
    await message.answer(
        f"Buyurtmangiz qabul qilindi!\n\n{duration} Telegram Premium\n"
        f"Admin chekni ko'rib Premium ulanadi.",
        reply_markup=get_main_menu()
    )
    await state.clear()
...
async def premium_approve_cb(call: CallbackQuery):
    ...
    if order:
        ...
        await bot.send_message(
            user_id,
            f"Tabriklaymiz! Telegram Premium ulandi!\n\n"
            f"{order['duration']} Premium obuna\n"
            f"Profil: @{order['target_username']}\n\nXarid uchun rahmat!",
            reply_markup=get_main_menu()
        )
        caption = call.message.caption or ""        try:
...
async def approve_pay(call: CallbackQuery):
    ...
    await bot.send_message(
        user_id,
        "To'lovingiz tasdiqlandi!\n\n«E'lon berish» tugmasini bosing.",        reply_markup=get_main_menu()
    )
    try:
...
async def approve_ad(call: CallbackQuery):
    ...
    try:
        await bot.send_video(MAIN_CHANNEL_ID, video=ad["video_id"], caption=ad["text"], reply_markup=btn)
    except Exception as e:
...
async def send_reply(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(f"Xato: {e}", reply_markup=get_admin_menu())    await state.clear()
    ...
    for i, user in enumerate(users):
        ...
        try:
            if message.photo:
                ...
                await bot.send_document(uid, document=message.document.file_id, caption=message.caption or "")
            elif message.sticker:
                await bot.send_sticker(uid, sticker=message.sticker.file_id)
            elif message.animation:
                await bot.send_animation(uid, animation=message.animation.file_id, caption=message.caption or "")
            elif message.text:
...
async def admin_stats_btn(message: Message):
    ...
    await message.answer(
        ...
        f"E'lonlar: {ads_count} ta (tasdiqlangan: {ads_approved})\n"
        f"UC buyurtmalar: {uc_count} ta (tasdiqlangan: {uc_approved})\n"
        f"Stars buyurtmalar: {stars_count} ta\n"
        f"Premium buyurtmalar: {premium_count} ta\n"
...
async def add_uc_price_step2(message: Message, state: FSMContext):
    ...
    await message.answer(f"{message.text} UC narxini kiriting (so'mda)")
...
async def add_uc_price_save(message: Message, state: FSMContext):
    ...
    if existing:
        ...
        await message.answer(f"{uc_amount} UC narxi yangilandi: {price:,} so'm".replace(",", " "), reply_markup=get_uc_admin_menu())
    else:
        db_execute("INSERT INTO uc_prices (uc_amount, price) VALUES (?,?)", (uc_amount, price))
        await message.answer(f"{uc_amount} UC — {price:,} so'm qo'shildi!".replace(",", " "), reply_markup=get_uc_admin_menu())
    await state.clear()

@router.message(F.text == "UC narxlari", F.from_user.id == ADMIN_ID)async def admin_uc_list_btn(message: Message):
    prices = db_execute("SELECT * FROM uc_prices ORDER BY uc_amount ASC", fetch=True)
    if not prices:
    ...
    for item in prices:
        ...
        rows.append([
            InlineKeyboardButton(text=f"{item['uc_amount']} UC", callback_data="uc_info", style="primary"),
...
async def admin_clear_uc_btn(message: Message):
    ...
    await message.answer("Barcha UC narxlarini o'chirasizmi?", reply_markup=btn)
...
async def add_stars_price_step2(message: Message, state: FSMContext):
    if not message.text.isdigit():        await message.answer("Faqat raqam kiriting!")
        return
    await state.update_data(stars_amount=int(message.text))
    await message.answer(f"{message.text} Stars narxini kiriting (so'mda)")
...
async def add_stars_price_save(message: Message, state: FSMContext):
    ...
    if existing:
        ...
        await message.answer(f"{stars_amount} Stars narxi yangilandi: {price:,} so'm".replace(",", " "), reply_markup=get_stars_admin_menu())
    else:
        db_execute("INSERT INTO stars_prices (stars_amount, price) VALUES (?,?)", (stars_amount, price))
        await message.answer(f"{stars_amount} Stars — {price:,} so'm qo'shildi!".replace(",", " "), reply_markup=get_stars_admin_menu())
    await state.clear()
...
async def admin_stars_orders_btn(message: Message):
    ...
    for o in orders:
        emoji = "⏳" if o["status"] == "pending" else ("✅" if o["status"] in ("approved", "payment_confirmed") else "❌")
        method = "🤖" if o["payment_method"] == "auto" else "👤"        text += f"{emoji}{method} #{o['id']} | {o['full_name']} | {o['stars_amount']} Stars | @{o['target_username']} | {o['order_date']}\n"
    await message.answer(text)
...
async def admin_clear_stars_btn(message: Message):
    ...
    await message.answer("Barcha Stars narxlarini o'chirasizmi?", reply_markup=btn)
...
async def add_premium_price_step2(message: Message, state: FSMContext):
    ...
    await message.answer(f"«{message.text}» narxini kiriting (so'mda)")
...
async def add_premium_price_save(message: Message, state: FSMContext):
    ...
    await message.answer(f"{duration} — {price:,} so'm qo'shildi!".replace(",", " "), reply_markup=get_premium_admin_menu())
    await state.clear()
...
async def admin_premium_list_btn(message: Message):
    ...
    for item in prices:
        ...
        rows.append([            InlineKeyboardButton(text=f"{item['duration']}", callback_data="premium_info", style="primary"),
...
async def admin_clear_premium_btn(message: Message):
    ...
    await message.answer("Barcha Premium narxlarini o'chirasizmi?", reply_markup=btn)
...
async def info_cb(call: CallbackQuery):    await call.answer()
...
async def del_uc_price(call: CallbackQuery):
    ...
    if item:
        ...
        for p in prices:
            ...
            rows.append([
                InlineKeyboardButton(text=f"{p['uc_amount']} UC", callback_data="uc_info", style="primary"),
...
async def del_stars_price(call: CallbackQuery):
    ...
    if item:
        ...
        for p in prices:
            ...
            rows.append([
                InlineKeyboardButton(text=f"{p['stars_amount']} Stars", callback_data="stars_info", style="primary"),
...
async def del_premium_price(call: CallbackQuery):
    ...
    if item:
        ...
        for p in prices:
            ...
            rows.append([
                InlineKeyboardButton(text=f"{p['duration']}", callback_data="premium_info", style="primary"),
                InlineKeyboardButton(text="O'chirish", callback_data=f"del_premium_price_{p['id']}", style="danger"),
            ])
        rows.append([InlineKeyboardButton(text="Yopish", callback_data="close_list", style="danger")])        try:
            await call.message.edit_text(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=rows))
        except:
            pass
    else:
...
async def check_bot_status(message: Message):
    ...
    try:
        ...
        can_post = getattr(member, 'can_post_messages', False)        status_text = "Hammasi yaxshi!" if can_post else "Botni kanalga ADMIN qilib qo'shing!"
        await message.answer(
            f"Bot: @{me.username}\n"
            f"Kanal: {MAIN_CHANNEL_ID}\n"
            f"Status: {member.status}\n"
            f"Post yuborish: {'Ha' if can_post else 'Yo'q'}\n\n"
            f"{status_text}"
        )
    except Exception as e:
