...
async def menu_ad_cb(message: Message, state: FSMContext):
    ...
    # Aks holda — to'lov so'ra
    btn = InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(text=f"💳 To'lov qilish ({price_int:,} so'm)".replace(",", " "), callback_data="pay_ad_auto", style="success")
    ]])
    await message.answer(
        f"📝 <b>E'lon joylash</b>\n\n"
        f"💰 Har bir e'lon narxi: <b>{price_int:,} so'm</b>\n\n".replace(",", " ") +
        f"E'lon joylash uchun avval to'lov qiling.\n"
        f"To'lov avtomatik tasdiqlanadi va e'lon boshlaydi.",
        reply_markup=btn,
        parse_mode="HTML"
    )
...
async def pay_ad_auto_cb(call: CallbackQuery, state: FSMContext):
    ...
    if result.get("success") or result.get("payment_url") or result.get("url"):
        ...
        if pay_url:
            ...
            await _fallback_manual_payment(call, price_int, data)
    else:
        # Checkout.uz ulanmagan bo'lsa — manual to'lov
        await _fallback_manual_payment(call, price_int, data)

async def _fallback_manual_payment(call, price_int, data):
    """Checkout.uz ishlamasa manual to'lov"""
...
async def handle_checkout_webhook(request: web.Request) -> web.Response:
    ...
    if status in ("paid", "success", "completed"):
        ...
        if payment and payment["status"] == "pending":
            ...
            if payment_type == "ad":
                # Foydalanuvchiga paid_slots qo'shish
                users = data.get("users", [])
                for u in users:
...
async def get_phone(message: Message, state: FSMContext):
    ...
    for u in users:
        if u["user_id"] == message.from_user.id:
            ...
            # paid_slots ni kamaytirish (sarflangan)
...
# ================== ADMIN TO'LOV MANUAL TASDIQLASH ==================
...
# Admin menyu tugmalari handlerlari
...
async def main():
    ...
    # Web server va botni birga ishlatish
